...
router.get('/filter', (req, res) => {
    ...
    let firstLetter = req.query.letter;
    ...
    if(firstLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name.startsWith(firstLetter))
    }
...
router.get('/:id', (req, res) => {
    ...
    friend? res.json({result: 'Finding friend with ID ' + friendId + ' ' + friend.name}): 
...
router.put('/:id', (req, res) => {
    ...
    if(friendIndex === -1) {
        res.status(404).send('friend coould not be found.')
        return
    }
